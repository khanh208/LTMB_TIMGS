...
const createRequest = async (studentId, tutorId, message) => {
  ...
  // (Bạn có thể thêm logic kiểm tra xem đã gửi request trước đó chưa)
...
const updateMyProfile = async (userId, updates) => {
  const { bio, price_per_hour, subjects } = updates;

  const updatedProfile = await tutorModel.updateProfile(userId, { bio, price_per_hour });

  if (Array.isArray(subjects)) {
    await tutorModel.updateSubjects(userId, subjects);
  }
